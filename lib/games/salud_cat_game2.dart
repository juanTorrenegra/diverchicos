...
const String kSaludCatMouthRinseAsset = 'assets/video/salud/catMouthRinse.mp4';
...
/// Cat bath phase 2: mouth rinse, soap scrub, triangle water drops.
///
/// Started from [SaludCatGameLayer] when the water-pour cue circle is tapped.
class SaludCatGame2Layer extends StatefulWidget {
  const SaludCatGame2Layer({
    super.key,
    this.onMouthRinseStarted,
    required this.onPhaseComplete,
  });

  /// Called once when [catMouthRinse.mp4] begins playing.
  final VoidCallback? onMouthRinseStarted;

  /// Called after soap and triangle are disposed (before cat ending).
  final VoidCallback onPhaseComplete;

  @override
  State<SaludCatGame2Layer> createState() => _SaludCatGame2LayerState();
}

class _SaludCatGame2LayerState extends State<SaludCatGame2Layer>
    with TickerProviderStateMixin {
  static const double _kLogicalH = 1080;

  static const Offset _kBathSoapPos = Offset(1200.9, 149.9);

  static const Offset _kLayoutCuePos = Offset(745.3, 305.0);
  static const double _kLayoutCueW = 350;
  /// Cow scrub zone height (850) reduced 30% for smaller cat.
  static const double _kLayoutCueH = 595;
  ...
  bool _mouthRinseStartedNotified = false;
  ...
  Future<void> _startMouthRinseVideo() async {
    ...
    try {
      ...
      if (!_mouthRinseStartedNotified) {
        _mouthRinseStartedNotified = true;
        widget.onMouthRinseStarted?.call();
      }
      setState(() {
