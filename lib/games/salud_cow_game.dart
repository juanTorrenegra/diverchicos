...
/// Spine-style strips: **[jumping]** (~1 s per full 24‑frame cycle at [_kCowAnimStepTime]) ×3,
/// then **[jumpToIdle]** once, then static **`cow.png`**. Keys live under `assets/images/`.
...
/// Rows may name frames `cow` or parse to empty/`0`; try `cow`, then blanks, then atlas order.
const String kCowAtlasNamePreferred = 'cow';

/// Three full hopping cycles on walk‑in (~1 s each at 24 logical fps).
const int kCowJumpLoopsOnEnter = 3;

/// 24 frames → 1 s per hopping cycle (user spec).
const double _kCowAnimStepTime = 1 / 24;

/// Logical draw size for the cow on screen (square); tune if clip / door layout needs it.
const double kCowSaludDrawSizePx = 800;
...
class SaludCowTuning {
  const SaludCowTuning({
    ...
    this.cowExitEndX =
        1728, // TWEAK: X destination after tap (walk out phase)
...
enum _CowPhase { entering, idle, exiting, done }
...
/// Ordered frame sprites (TexturePacker `index:` or `cow_*` names).
List<Sprite> _cowFramesFromAtlas(
  ...
  var frames = pick(kCowAtlasNamePreferred);
  if (frames.isEmpty) {
    frames = pick('');
  }
  if (frames.isEmpty) {
  ...
  assert(
    frames.isNotEmpty,
    'No frames in $atlasLabelForAssert (cow / blank name / atlas.sprites)',
  );
  return List<Sprite>.from(frames);
}

SpriteAnimation _spriteAnimFromSprites(
  List<Sprite> frames, {
  required bool loop,
}) {
  return SpriteAnimation.spriteList(frames, stepTime: _kCowAnimStepTime, loop: loop);
}

SpriteAnimation _composeEnterWalkAnimation(List<Sprite> jump, List<Sprite> toIdle) {
  final sprites = <Sprite>[];
  for (var loop = 0; loop < kCowJumpLoopsOnEnter; loop++) {
    sprites.addAll(jump);
  }
  sprites.addAll(toIdle);
  return _spriteAnimFromSprites(sprites, loop: false);
}
...
final class SaludCowActor extends PositionComponent
  ...
  /// True after jump×3 + jump‑to‑idle finished during [entering].
  ...
  Future<void> _firstPaint() async {
    ...
    final jumpFrames = _cowFramesFromAtlas(
      jumpAtlas,
      atlasLabelForAssert: kCowJumpingAtlasAsset,
    );
    final jumpToIdleFrames = _cowFramesFromAtlas(
      idleAtlas,
      atlasLabelForAssert: kCowJumpToIdleAtlasAsset,
    );

    _jumpingLoopTemplate = _spriteAnimFromSprites(jumpFrames, loop: true);
    _enterOnceTemplate = _composeEnterWalkAnimation(
      jumpFrames,
      jumpToIdleFrames,
    );

    final idleSprite = await game.loadSprite(kCowStaticPngAsset);
    _staticTemplate = _spriteAnimFromSprites([idleSprite], loop: true);
    ...
    unawaited(
      enterTicker.completed.then((_) {
        if (!isMounted || !_visualReady || _phase != _CowPhase.entering) return;
  ...
  Vector2 _cowDisplaySize() => Vector2.all(
    kCowSaludDrawSizePx.clamp(1, math.max(1, kSaludCowLogicalHeight)),
  );
  ...
  void _trySettleIdleAtCenter() {
    if (_phase != _CowPhase.entering) return;
